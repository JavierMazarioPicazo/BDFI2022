...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Change base path to point to the absolute path of the container
    val base_path= "/app/practica_big_data_2019"
    ...
    // Change kafka server to kafka container
    val df = spark
      .readStream
      .format("kafka")
    ...
    // Change mongo url to mongo container
    val writeConfig = WriteConfig(Map("uri" -> "mongodb://mongod:27017/agile_data_science.flight_delay_classification_response"))
